...
/mob/living/carbon/human/proc/rebuild_clothing_overlays()
	...
	if(src.gender == "female")
		if(!lying)
			...
			if(istype(w_uniform, /obj/item/clothing/under))
				...
				if (!t1)
					t1 = icon_state
				clothing_overlays += image("icon" = 'uniform_f.dmi', "icon_state" = "[t1]_s", "layer" = UNIFORM_LAYER)
			...
			if (wear_id)
				if(wear_id.over_jumpsuit)
					clothing_overlays += image("icon" = 'mob.dmi', "icon_state" = "id", "layer" = ID_LAYER)
	...
	//					t1 = gloves.icon_state
					var/icon/gloves_icon = new /icon("icon" = 'hands_f.dmi', "icon_state" = gloves.item_state)
					if(lo.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "right_s"), ICON_MULTIPLY)
					if(ro.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "left_s"), ICON_MULTIPLY)
					clothing_overlays += image(gloves_icon, "layer" = GLOVES_LAYER)
					if (gloves.blood_DNA)
						var/icon/stain_icon = icon('blood.dmi', "bloodyhands")
						if(lo.status & DESTROYED)
							stain_icon.Blend(new /icon('limb_mask.dmi', "right_s"), ICON_MULTIPLY)
						else if(ro.status & DESTROYED)
							stain_icon.Blend(new /icon('limb_mask.dmi', "left_s"), ICON_MULTIPLY)
						clothing_overlays += image("icon" = stain_icon, "layer" = B_GLOVES_LAYER)
				else if (blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "bloodyhands2")
					if(lo.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "right_s"), ICON_MULTIPLY)
					else if(ro.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "left_s"), ICON_MULTIPLY)
	...
	// Glasses
			if (glasses)
				...
				clothing_overlays += image("icon" = 'eyes_f.dmi', "icon_state" = glasses.icon_state, "layer" = GLASSES_LAYER)
	...
	// Shoes
			...
			//	var/t1 = shoes.icon_state
				var/icon/shoes_icon = new /icon("icon" = 'feet_f.dmi', "icon_state" = shoes.icon_state)
				if(lo.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "right"), ICON_MULTIPLY)
				else if(ro.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "left"), ICON_MULTIPLY)
				clothing_overlays += image(shoes_icon, "layer" = SHOES_LAYER)
				if (shoes.blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "shoeblood")
					if(lo.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "right_s"), ICON_MULTIPLY)
					else if(ro.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "left_s"), ICON_MULTIPLY)
			...
			//		t1 = s_store.icon_state
				if(!istype(wear_suit, /obj/item/clothing/suit/storage/armoredundersuit))
					clothing_overlays += image("icon" = 'belt_mirror.dmi', "icon_state" = s_store.item_state, "layer" = SUIT_STORE_LAYER)
			...
			if (wear_mask)
				...
				//	var/t1 = wear_mask.icon_state
					clothing_overlays += image("icon" = 'mask.dmi', "icon_state" = wear_mask.icon_state, "layer" = FACEMASK_LAYER)
	...
	// Head
			...
			//	var/t1 = head.icon_state
				var/icon/head_icon = icon('head.dmi', head.icon_state)
				if(istype(head,/obj/item/clothing/head/kitty))
					head_icon = head:mob
				clothing_overlays += image("icon" = head_icon, "layer" = HEAD_LAYER)
				if(gimmick_hat)
					clothing_overlays += image("icon" = icon('gimmick_head.dmi', "[gimmick_hat]"), "layer" = HEAD_LAYER)
				if (head.blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "helmetblood")
	...
	// Belt
			...
			//	if (!t1)
			//		t1 = belt.icon_state
				clothing_overlays += image("icon" = 'belt_f.dmi', "icon_state" = belt.item_state, "layer" = BELT_LAYER)
			...
			//	var/t1 = back.icon_state
				clothing_overlays += image("icon" = 'back_f.dmi', "icon_state" = back.icon_state, "layer" = BACK_LAYER)
			...
			if (handcuffed)
				...
				clothing_overlays += image("icon" = 'mob_f.dmi', "icon_state" = "[handcuffed.icon_state]1", "layer" = CUFFED_LAYER)
	...
	// Splints
			for(var/organ in list("l_leg","r_leg","l_arm","r_arm"))
				var/datum/organ/external/o = organs["[organ]"]
				if (o.status & SPLINTED)
					clothing_overlays += image("icon" = 'mob_f.dmi', "icon_state" = "[o]_splint", "layer" = CUFFED_LAYER)

			if (r_hand)
				clothing_overlays += image("icon" = 'items_righthand.dmi', "icon_state" = r_hand.item_state, "layer" = INHANDS_LAYER)
				r_hand.screen_loc = ui_rhand

			if (l_hand)
				clothing_overlays += image("icon" = 'items_lefthand.dmi', "icon_state" = l_hand.item_state, "layer" = INHANDS_LAYER)
			...
			if (wear_suit)
				...
				//	var/t1 = wear_suit.icon_state
					clothing_overlays += image("icon" = 'suit_f.dmi', "icon_state" = wear_suit.icon_state, "layer" = SUIT_LAYER)
...
*/
	...
	/////////////////////////
			...
			if(istype(w_uniform, /obj/item/clothing/under))
				...
				if (!t1)
					t1 = icon_state
				clothing_overlays += image("icon" = 'uniform_f.dmi', "icon_state" = "[t1]_l", "layer" = UNIFORM_LAYER)
				if (w_uniform.blood_DNA)
			...
			if (wear_id)
				if(wear_id.over_jumpsuit)
					clothing_overlays += image("icon" = 'mob.dmi', "icon_state" = "id2", "layer" = ID_LAYER)
		...
		// Gloves
			...
			if (!(lo.status & DESTROYED && ro.status & DESTROYED))
				...
				//	if (!t1)
				//		t1 = gloves.icon_state
					var/icon/gloves_icon = new /icon("icon" = 'hands_f.dmi', "icon_state" = "[gloves.item_state]2")
					if(lo.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
					if(ro.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
					clothing_overlays += image(gloves_icon, "layer" = GLOVES_LAYER)
					if (gloves.blood_DNA)
						var/icon/stain_icon = icon('blood.dmi', "bloodyhands2")
						if(lo.status & DESTROYED)
							stain_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
						else if(ro.status & DESTROYED)
							stain_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
						clothing_overlays += image("icon" = stain_icon, "layer" = B_GLOVES_LAYER)
				else if (blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "bloodyhands2")
					if(lo.status & DESTROYED)
		...
		// Glasses
			if (glasses)
			//	var/t1 = glasses.icon_state
				clothing_overlays += image("icon" = 'eyes_f.dmi', "icon_state" = "[glasses.icon_state]2", "layer" = GLASSES_LAYER)

		// Ears
			if (l_ear)
			//	var/t1 = l_ear.icon_state
				clothing_overlays += image("icon" = 'ears_f.dmi', "icon_state" = "[l_ear.icon_state]2", "layer" = EARS_LAYER)
			if (r_ear)
			//	var/t1 = r_ear.icon_state
				clothing_overlays += image("icon" = 'ears_f.dmi', "icon_state" = "[r_ear.icon_state]2", "layer" = EARS_LAYER)
		...
		// Shoes
			...
			if (!(lo.status & DESTROYED && ro.status & DESTROYED) && shoes)
			//	var/t1 = shoes.icon_state
				var/icon/shoes_icon = new /icon("icon" = 'feet_f.dmi', "icon_state" = "[shoes.icon_state]2")
				if(lo.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
				else if(ro.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
				clothing_overlays += image(shoes_icon, "layer" = SHOES_LAYER)
				if (shoes.blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "shoeblood2")
					if(lo.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
					else if(ro.status & DESTROYED)
						stain_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
			...
			//		t1 = s_store.icon_state
				if(!istype(wear_suit, /obj/item/clothing/suit/storage/armoredundersuit))
					clothing_overlays += image("icon" = 'belt_mirror.dmi', "icon_state" = "[s_store.item_state]2", "layer" = SUIT_STORE_LAYER)
				s_store.screen_loc = ui_sstore1

			if (wear_mask)
				if (istype(wear_mask, /obj/item/clothing/mask))
				//	var/t1 = wear_mask.icon_state
					clothing_overlays += image("icon" = 'mask.dmi', "icon_state" = "[wear_mask.icon_state]2", "layer" = FACEMASK_LAYER)
					if (!istype(wear_mask, /obj/item/clothing/mask/cigarette) && (wear_mask.blood_DNA))
						var/icon/stain_icon = icon('blood.dmi', "maskblood2")
						clothing_overlays += image("icon" = stain_icon, "layer" = B_FACEMASK_LAYER)
						wear_mask.screen_loc = ui_mask
		...
		// Head
			if (head)
			//	var/t1 = head.icon_state
				var/icon/head_icon = icon('head.dmi', "[head.icon_state]2")
				if(istype(head,/obj/item/clothing/head/kitty))
					head_icon = head:mob2
				clothing_overlays += image("icon" = head_icon, "layer" = HEAD_LAYER)
				if(gimmick_hat)
					clothing_overlays += image("icon" = icon('gimmick_head.dmi', "[gimmick_hat]2"), "layer" = HEAD_LAYER)
				if (head.blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "helmetblood2")
					clothing_overlays += image("icon" = stain_icon, "layer" = B_HEAD_LAYER)
				head.screen_loc = ui_head
		...
		// Belt
			...
			//	if (!t1)
			//		t1 = belt.icon_state
				clothing_overlays += image("icon" = 'belt_f.dmi', "icon_state" = "[belt.item_state]2", "layer" = BELT_LAYER)
				belt.screen_loc = ui_belt
			...
			if (back)
			//	var/t1 = back.icon_state
				clothing_overlays += image("icon" = 'back_f.dmi', "icon_state" = "[back.icon_state]2", "layer" = BACK_LAYER)
				back.screen_loc = ui_back
			...
			if (handcuffed)
				pulling = null
			//	var/h1 = handcuffed.icon_state
				clothing_overlays += image("icon" = 'mob_f.dmi', "icon_state" = "[handcuffed.icon_state]2", "layer" = CUFFED_LAYER)
		...
		// Splints
			...
			if (wear_suit)
				if (istype(wear_suit, /obj/item/clothing/suit))
				//	var/t1 = wear_suit.icon_state
					clothing_overlays += image("icon" = 'suit_f.dmi', "icon_state" = "[wear_suit.icon_state]2", "layer" = SUIT_LAYER)
				if (istype(wear_suit, /obj/item/clothing/suit/straight_jacket))
					if (handcuffed)
						handcuffed.loc = loc
						handcuffed.layer = initial(handcuffed.layer)
				...
				if (wear_suit.blood_DNA)
					var/icon/stain_icon = null
					if (istype(wear_suit, /obj/item/clothing/suit/armor/vest || /obj/item/clothing/suit/storage/wcoat))
						stain_icon = icon('blood.dmi', "armorblood2")
					else if (istype(wear_suit, /obj/item/clothing/suit/storage/det_suit || /obj/item/clothing/suit/storage/labcoat))
						stain_icon = icon('blood.dmi', "coatblood2")
					else
						stain_icon = icon('blood.dmi', "suitblood2")
					clothing_overlays += image("icon" = stain_icon, "layer" = B_SUIT_LAYER)
				wear_suit.screen_loc = ui_oclothing
...
///////////////////////////////////////////////////////////////////////
	...
	if(src.gender == "male")
		if(!lying)
			...
			if(istype(w_uniform, /obj/item/clothing/under))
				...
				if (!t1)
					t1 = icon_state
				clothing_overlays += image("icon" = 'uniform.dmi', "icon_state" = "[t1]_s", "layer" = UNIFORM_LAYER)
				if (w_uniform.blood_DNA)
					var/icon/stain_icon = icon('blood.dmi', "uniformblood")
					clothing_overlays += image("icon" = stain_icon, "layer" = B_UNIFORM_LAYER)
	...
	//				if (!t1)
	//					t1 = gloves.icon_state
					var/icon/gloves_icon = new /icon("icon" = 'hands.dmi', "icon_state" = gloves.item_state)
					if(lo.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "right_s"), ICON_MULTIPLY)
					if(ro.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "left_s"), ICON_MULTIPLY)
	...
	// Glasses
			if (glasses)
				//var/t1 = glasses.icon_state
				clothing_overlays += image("icon" = 'eyes.dmi', "icon_state" = glasses.icon_state, "layer" = GLASSES_LAYER)
	...
	// Shoes
			...
			if (!(lo.status & DESTROYED && ro.status & DESTROYED) && shoes)
			//	var/t1 = shoes.icon_state
				var/icon/shoes_icon = new /icon("icon" = 'feet.dmi', "icon_state" = shoes.icon_state)
				if(lo.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "right"), ICON_MULTIPLY)
				else if(ro.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "left"), ICON_MULTIPLY)
	...
	// Belt
			...
			//	if (!t1)
			//		t1 = belt.icon_state
				clothing_overlays += image("icon" = 'belt.dmi', "icon_state" = belt.item_state, "layer" = BELT_LAYER)
				belt.screen_loc = ui_belt
			...
			if (back)
			//	var/t1 = back.icon_state
				clothing_overlays += image("icon" = 'back.dmi', "icon_state" = back.icon_state, "layer" = BACK_LAYER)
				back.screen_loc = ui_back
			...
			if (handcuffed)
				pulling = null
				//var/h1 = handcuffed.icon_state
				clothing_overlays += image("icon" = 'mob.dmi', "icon_state" = "[handcuffed.icon_state]1", "layer" = CUFFED_LAYER)
	...
	// Splints
			...
			if (wear_suit)
				if (istype(wear_suit, /obj/item/clothing/suit))
				//	var/t1 = wear_suit.icon_state
					clothing_overlays += image("icon" = 'suit.dmi', "icon_state" = wear_suit.icon_state, "layer" = SUIT_LAYER)
				if (istype(wear_suit, /obj/item/clothing/suit/straight_jacket))
					if (handcuffed)
						handcuffed.loc = loc
						handcuffed.layer = initial(handcuffed.layer)
	...
	/////////////////////////
		...
		// Gloves
			...
			if (!(lo.status & DESTROYED && ro.status & DESTROYED))
				...
				//	if (!t1)
				//		t1 = gloves.icon_state
					var/icon/gloves_icon = new /icon("icon" = 'hands.dmi', "icon_state" = "[gloves.item_state]2")
					if(lo.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
					if(ro.status & DESTROYED)
						gloves_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
		...
		// Glasses
			if (glasses)
			//	var/t1 = glasses.icon_state
				clothing_overlays += image("icon" = 'eyes.dmi', "icon_state" = "[glasses.icon_state]2", "layer" = GLASSES_LAYER)

		// Ears
			if (l_ear)
			//	var/t1 = l_ear.icon_state
				clothing_overlays += image("icon" = 'ears.dmi', "icon_state" = "[l_ear.icon_state]2", "layer" = EARS_LAYER)
			if (r_ear)
			//	var/t1 = r_ear.icon_state
				clothing_overlays += image("icon" = 'ears.dmi', "icon_state" = "[r_ear.icon_state]2", "layer" = EARS_LAYER)
		...
		// Shoes
			...
			if (!(lo.status & DESTROYED && ro.status & DESTROYED) && shoes)
			//	var/t1 = shoes.icon_state
				var/icon/shoes_icon = new /icon("icon" = 'feet.dmi', "icon_state" = "[shoes.icon_state]2")
				if(lo.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "right_l"), ICON_MULTIPLY)
				else if(ro.status & DESTROYED && !lying)
					shoes_icon.Blend(new /icon('limb_mask.dmi', "left_l"), ICON_MULTIPLY)
		...
		// Belt
			...
			//	if (!t1)
			//		t1 = belt.icon_state
				clothing_overlays += image("icon" = 'belt.dmi', "icon_state" = "[belt.item_state]2", "layer" = BELT_LAYER)
				belt.screen_loc = ui_belt
			...
			if (back)
			//	var/t1 = back.icon_state
				clothing_overlays += image("icon" = 'back.dmi', "icon_state" = "[back.icon_state]2", "layer" = BACK_LAYER)
				back.screen_loc = ui_back
			...
			if (handcuffed)
				pulling = null
			//	var/h1 = handcuffed.icon_state
				clothing_overlays += image("icon" = 'mob.dmi', "icon_state" = "[handcuffed.icon_state]2", "layer" = CUFFED_LAYER)
		...
		// Splints
			...
			if (wear_suit)
				if (istype(wear_suit, /obj/item/clothing/suit))
				//	var/t1 = wear_suit.icon_state
					clothing_overlays += image("icon" = 'suit.dmi', "icon_state" = "[wear_suit.icon_state]2", "layer" = SUIT_LAYER)
				if (istype(wear_suit, /obj/item/clothing/suit/straight_jacket))
					if (handcuffed)
						handcuffed.loc = loc
						handcuffed.layer = initial(handcuffed.layer)
