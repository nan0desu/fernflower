	...
	Bump(atom/A as mob|obj|turf|area)
		...
		if(firer && istype(A, /mob))
			...
			if(istype(firer, /mob))
				...
				message_admins("ATTACK: [firer] ([firer.ckey])(<A HREF='?src=\ref[src];adminplayerobservejump=\ref[firer]'>JMP</A>) shot [M] ([M.ckey]) with [src].")
				...
				message_admins("ATTACK: UNKNOWN (no longer exists) shot [M] ([M.ckey])(<A HREF='?src=\ref[src];adminplayerobservejump=\ref[M]'>JMP</A>) with [src].")
