...
/obj/item/weapon/flashbang
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob)
		...
		if((user.equipped() == src)&&(!active)&&(clown_check(user)))
			...
			message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?src=\ref[src];adminplayerobservejump=\ref[user]'>JMP</A>) primed a flashbang.")
...
/obj/item/weapon/flashbang/clusterbang
	...
	attack_self(mob/user as mob)
		if(!active)
			...
			message_admins("ATTACK: [user] ([user.ckey])(<A HREF='?src=\ref[src];adminplayerobservejump=\ref[user]'>JMP</A>) primed a [src].")
			src.active = 1
