...
var/global/list/space_surprises = list(		/obj/alien/facehugger		=4,
											...
											/obj/item/clothing/glasses/thermal				=1,	// Could maybe be cool as its own rapid mode, sorta like wizard.  Maybe.
											/obj/item/weapon/storage/box/stealth/			=1
	...
	for(var/x = 0,x<x_size,x++)
		for(var/y = 0,y<y_size,y++)
			...
			if(x == 0 || x==x_size-1 || y==0 || y==y_size-1)
				if(wall == /obj/alien/resin)
					T = new floor(cur_loc)
					new /obj/alien/resin(T)
				else
					T = new wall(cur_loc)
					room_turfs["walls"] += T
			else
